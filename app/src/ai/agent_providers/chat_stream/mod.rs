...
use readiness::{
    accepted_history_repair_log_message, build_serializer_readiness_projection,
    classify_byop_controller_readiness, classify_byop_controller_readiness_with_live_tool_calls,
    current_input_result_kind, current_input_task_id, persisted_tool_result_kind,
    redacted_tool_kind_for_tool_call, validate_byop_serializer_readiness,
    validate_serializer_readiness_projection,
    validate_serializer_readiness_projection_with_repair_state,
};
use diagnostics::{
    log_chat_request_details, snippet_for_log, BYOP_DIAG_SNIPPET_CHARS,
};

/// 兜底:确保 messages 末尾是 user(或 tool 响应)。
///
/// 触发场景:`AIAgentInput::ResumeConversation` 不附加新 user 消息,直接重发历史。
/// Anthropic 原生 API 拒绝末尾为 assistant 的请求,
