...
/// Marb:渲染 SSH 会话状态块,append 到 system prompt 末尾。
///
...
fn build_user_message_with_binaries(
    ...
    for bin in binaries {
        if !caps.supports_mime(&bin.content_type) {
            // Marb 对齐 opencode `unsupportedParts`(packages/opencode/src/provider/transform.ts:305-341):
...
/// Marb Drive 本地文档存储(AIDocumentModel),不碰文件系统、不跑命令,语义上
...
fn build_tools_array(params: &RequestParams) -> Vec<GenaiTool> {
    // Marb A2:LRC tag-in 场景剔除 `run_shell_command`,迫使模型选 PTY 操作类工具。
    //
    ...
    let web_enabled = params.web_search_enabled;
    let plan_mode = is_plan_mode_turn(&params.input);
    // Marb BYOP:`suggest_prompt` chip UI 已通过 view 层订阅
    ...
    let mut out: Vec<GenaiTool> = tools::REGISTRY
        ...
        .filter(|t| {
            ...
            // suggest_new_conversation:无 UI 实现,executor 在 Marb 改为
...
/// 中转 / 本地服务 / 大部分国内云)一律不发。
///
/// Marb 这边没有 `providerID` 这个维度,只有用户自由填写的 `base_url`。
/// 因此通过 `base_url` 反推:
...
fn build_chat_options(
    ...
    //
    //    opencode 用 `providerID`(用户 config 选的字符串)做判定;Marb 这边没有
    ...
    //   需要显式 `extra_body.thinking.type=disabled` 才能关闭。Marb 本地 fork
