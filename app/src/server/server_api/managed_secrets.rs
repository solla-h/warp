...
impl ManagedSecretsClient for ServerApi {
    async fn get_managed_secret_configs(&self) -> Result<ManagedSecretConfigs> {
        todo!("get_managed_secret_configs: GraphQL backend removed, needs REST migration")
    }

    async fn delete_managed_secret(&self, _owner: SecretOwner, _name: String) -> Result<()> {
        todo!("delete_managed_secret: GraphQL backend removed, needs REST migration")
    }
    ...
    async fn issue_task_identity_token(
        ...
        todo!("issue_task_identity_token: GraphQL backend removed, needs REST migration")
    }
}
