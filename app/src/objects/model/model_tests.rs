...
fn test_update_with_deleted_objects() {
    ...
    App::test((), |mut app| async move {
        ...
        cloud_model.read(&app, |cloud_model, _| {
            ...
            assert_eq!(7, cloud_model.as_object_types().count());
        });
        ...
        cloud_model.read(&app, |cloud_model, _| {
            ...
            assert_eq!(5, cloud_model.as_object_types().count());
        });
    })
}
...
fn test_load_object_types_on_initial_load_with_empty_cache() {
...
#[test]
fn test_loading_all_object_types_after_switching_from_offline() {
    let _flag = FeatureFlag::KnowledgeSidebar.override_enabled(true);
    ...
    App::test((), |mut app| async move {
        ...
        // next etc.)
        let mut object_types_sequence = Sequence::new();
        ...
        cloud_object_server_api_mock
            ...
            .in_sequence(&mut object_types_sequence)
            .return_once(move |_, _| {
                Ok(InitialLoadResponse {
...
fn test_force_refresh_only_happens_once() {
    ...
    App::test((), |mut app| async move {
        ...
        CloudModel::handle(&app).read(&app, |model, _ctx| {
            assert!(!model.object_types_force_refresh_pending())
        });
    })
}
...
fn test_shared_object_in_unshared_folder() {
    ...
    App::test((), |mut app| async move {
        ...
        CloudModel::handle(&app).update(&mut app, |cloud_model, ctx| {
            ...
            assert!(cloud_model
                .active_object_types_in_space(Space::Shared, ctx)
            ...
            assert_eq!(
                cloud_model
                    .trashed_object_types_in_space(Space::Shared, ctx)
                    .count(),
                0
            );
            assert_eq!(
                cloud_model
                    .trashed_object_types_in_location_without_descendents(
                        CloudObjectLocation::Space(Space::Shared),
                        ctx
                    )
                    .count(),
            ...
            assert_eq!(
                cloud_model
                    .active_object_types_in_location_without_descendents(folder_location, ctx)
                    .count(),
                0
            );
            assert_eq!(
                cloud_model
                    .trashed_object_types_in_location_without_descendents(folder_location, ctx)
                    .count(),
                0
            );
        });
