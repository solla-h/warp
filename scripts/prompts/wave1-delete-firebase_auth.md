# Wave 1: Delete Crate firebase_auth
# Issue: https://github.com/solla-h/warp/issues/5
# Copy this entire file content as your prompt in a new session with /implement

/implement https://github.com/solla-h/warp/issues/5
...
pwd
# Expected: a path ending in `wave1-firebase-auth` (NOT the main warp repo)

# 2. Confirm you are on the correct branch:
git branch --show-current
# Expected: wave1/delete-firebase-auth
...
Physically delete the `firebase_auth` crate:
...
4. **Delete the directory**: `rm -rf crates/firebase_auth/`
...
### Extra notes
- Very few app-level references — most usage is inside other cloud crates
- This should be one of the easiest deletions
...
- No references to `firebase_auth` remain in any Cargo.toml
...
- One commit per crate, clean commit message: "feat(strip): delete crate firebase_auth"
