# Wave 1: Delete Crate Template
# This is a TEMPLATE prompt. Replace {CRATE_NAME}, {ISSUE_URL}, {ISSUE_NUM}, and {NOTES} for each crate.
# Wave 1 crates (all parallel, pick any):
#   - #3: warp_server_client — {NOTES: ~10 files import fixup}
#   - #4: warp_server_auth — {NOTES: types already in warp_types, replace use warp_server_auth:: with use warp_types::}
#   - #5: firebase_auth — {NOTES: very few app references}
#   - #6: cloud_object_persistence — {NOTES: 100% cloud sync, few app refs}
#   - #7: warp_graphql — {NOTES: ~20 files import fixup, was already optional}
#   - #8: cloud_object_client — {NOTES: may not exist as separate crate, verify first}
#   - #9: managed_secrets_wasm + warp_managed_secrets — {NOTES: delete both, wasm+native pair}
...
Physically delete the `{CRATE_NAME}` crate:
...
4. **Delete the directory**: `rm -rf crates/{CRATE_NAME}/`
...
- No references to `{CRATE_NAME}` remain in any Cargo.toml
...
- One commit per crate, clean commit message: "feat(strip): delete crate {CRATE_NAME}"
