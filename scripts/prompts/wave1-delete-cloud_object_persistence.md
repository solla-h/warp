# Wave 1: Delete Crate cloud_object_persistence
# Issue: https://github.com/solla-h/warp/issues/6
# Copy this entire file content as your prompt in a new session with /implement

/implement https://github.com/solla-h/warp/issues/6
...
pwd
# Expected: a path ending in `wave1-cloud-object-persistence` (NOT the main warp repo)

# 2. Confirm you are on the correct branch:
git branch --show-current
# Expected: wave1/delete-cloud-object-persistence
...
Physically delete the `cloud_object_persistence` crate:
...
4. **Delete the directory**: `rm -rf crates/cloud_object_persistence/`
...
### Extra notes
- 100% cloud sync code — all 25 functions serve server sync lifecycle
- Zero local value: encode/decode_permissions, upsert/delete_cloud_object, mark_as_synced, etc.
- Should have very few app/ imports since it was already made optional
...
- No references to `cloud_object_persistence` remain in any Cargo.toml
...
- One commit per crate, clean commit message: "feat(strip): delete crate cloud_object_persistence"
