# Wave 2: Delete server/cloud_objects/ module
# Issue: https://github.com/solla-h/warp/issues/15
# Copy this entire file content as your prompt in a new session with /implement

/implement https://github.com/solla-h/warp/issues/15
...
- `app/src/server/mod.rs` — where the `mod cloud_objects` declaration lives
...
pwd
# Expected: a path ending in `wave2-server-cloud-objects` (NOT the main warp repo)

# 2. Confirm you are on the correct branch:
git branch --show-current
# Expected: wave2/delete-server-cloud-objects
...
Physically delete `app/src/server/cloud_objects/`:

1. **Delete the directory/file**: `rm -rf app/src/server/cloud_objects/`
2. **Remove mod declaration**: Delete `mod cloud_objects` from `app/src/server/mod.rs`
3. **Fix dead imports iteratively**:
   - Run `cargo check -p warp 2>&1 | head -50`
   - Fix errors (typically: delete `use crate::server::cloud_objects::*` lines)
...
### Extra notes
- ~110 files reference this module
- `UpdateManager` is the most commonly referenced type — remove all usage sites
- Some agent_sdk and terminal/view files reference UpdateManager — those are dead code paths in BYOP mode, delete them
- `CloudObjectStatuses::render_icon()` was the only reason this module pulled in UI deps — removing it is safe

## Workflow

```
rm -rf app/src/server/cloud_objects/
...
git add -A && git commit -m "feat(strip): delete cloud_objects module"
```
...
- No `use crate::server::cloud_objects::` references remain
- app/src/server/cloud_objects/ directory is gone
...
- One commit, message: "feat(strip): delete cloud_objects module"
