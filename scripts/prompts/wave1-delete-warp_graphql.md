# Wave 1: Delete Crate warp_graphql
# Issue: https://github.com/solla-h/warp/issues/7
# Copy this entire file content as your prompt in a new session with /implement

/implement https://github.com/solla-h/warp/issues/7
...
pwd
# Expected: a path ending in `wave1-warp-graphql` (NOT the main warp repo)

# 2. Confirm you are on the correct branch:
git branch --show-current
# Expected: wave1/delete-warp-graphql
...
Physically delete the `warp_graphql` crate:
...
4. **Delete the directory**: `rm -rf crates/warp_graphql/`
...
### Extra notes
- Expect ~20 files with dead imports to fix
- This crate was already made optional behind the 'cloud' feature in T6
- It generates GraphQL query types — any references to generated types (queries, mutations) should just be deleted
- Also remove the `cynic` dependency from app/Cargo.toml if it was only used via warp_graphql
...
- No references to `warp_graphql` remain in any Cargo.toml
...
- One commit per crate, clean commit message: "feat(strip): delete crate warp_graphql"
